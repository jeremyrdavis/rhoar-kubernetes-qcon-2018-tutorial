...
Let's add a check to verify the content returned by our server.  Add the following to line 29 just below the other "assert" statements:

[source, javascript]
```
assert res.result().toString().equalsIgnoreCase("Insult Service")
```
Run the test with 
...
The test should fail beacuase our MainVerticle is not returning the appropriate text.  If your test passes raise your hand and ask one of the Red Hat guys for help.  
